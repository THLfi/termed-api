...
import fi.thl.termed.spesification.lucene.ResourcesByClassId;

public class ResourcesByClassIdPermissionEvaluator
    implements PermissionEvaluator<ResourcesByClassId, Void> {
  ...
  public boolean hasPermission(User user, ResourcesByClassId specification, Permission permission) {
    ...
    for (SchemeRole schemeRole : user.getSchemeRoles()) {
      if (Objects.equal(schemeRole.getSchemeId(), classId.getSchemeId())) {
        if (classPermissionDao.exists(
            new ObjectRolePermission<ClassId>(classId, schemeRole.getRole(), permission))) {
          return true;
        }
      }
    }
