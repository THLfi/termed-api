...
public class JdbcTextAttributePermissionsDao
  ...
  protected RowMapper<ObjectRolePermission<TextAttributeId>> buildKeyMapper() {
    return (rs, rowNum) -> {
      SchemeId schemeId = new SchemeId(UUIDs.fromString(rs.getString("text_attribute_scheme_id")));
      ClassId domainId = new ClassId(rs.getString("text_attribute_domain_id"), schemeId);
