...
public class JdbcClassDao extends AbstractJdbcDao<ClassId, Class> {
  ...
  @Override
  protected <E> List<E> get(SqlSpecification<ClassId, Class> specification,
                            RowMapper<E> mapper) {
  ...
  protected <E> Optional<E> get(ClassId classId, RowMapper<E> mapper) {
    return jdbcTemplate.query(
        ...
        mapper, classId.getSchemeId(), classId.getId()).stream().findFirst();
  }

  @Override
  protected RowMapper<ClassId> buildKeyMapper() {
    return (rs, rowNum) -> new ClassId(UUIDs.fromString(rs.getString("scheme_id")),
                                       rs.getString("id"));
  }
