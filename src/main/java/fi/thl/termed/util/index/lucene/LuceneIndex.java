...
public class LuceneIndex<K extends Serializable, V> implements Index<K, V> {
  ...
  @Override
  public List<V> get(Query<K, V> query) {
    try {
  ...
  @Override
  public List<K> getKeys(Query<K, V> query) {
    try {
      IndexSearcher searcher = searcherManager.acquire();
      try {
        return query(searcher, getLuceneQuery(query), query.getMax(), query.getOrderBy(),
  ...
  private <E> List<E> query(IndexSearcher searcher, org.apache.lucene.search.Query query, int max,
