...
Properties can be queried by many different ways. Exact property match is done by
`where=properties.name:John`. Value must not contain whitespace. Phrase is matched by
`where=properties.name:"John Smith"`. Prefix search can be done with `\*` symbol, e.g.
`where=properties.name:Jo*`. Localized field is matched with `where=properties.prefLabel.en:Cat`.

Property fields in format `properties.<attr-id>` contain tokenized values. If un-tokenized match is
needed, field `properties.<attr-id>.string` can be used. For example exact match with property
containing exactly and only the queried value can be done with `properties.description.en.string:"Foo Bar"`.
When using tokenized match without `.string` suffix, `properties.description.en:"Foo Bar"` would
also match labels such as "Foo Bar Baz". Un-tokenized field can also be used in range queries such
as `properties.publishDate.string:[* TO 2019-08-01]` given field `publishDate` contains ISO-formatted
dates.
