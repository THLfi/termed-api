...
Node Tree API is can be used for as follows:

....
GET /api/node-trees?select=<fields>&where=<criteria>&sort=<fields>&max=<number>
....

Usually reads are limited to some graph or type:
...
A single tree can be loaded with:

....
GET /api/graphs/{graphId}/types/{typeId}/node-trees/{id}
....

If only node counts are needed `/api/node-count?where=<criteria>` can be used. Supported API for
...
Select lists fields that are returned on result objects.
