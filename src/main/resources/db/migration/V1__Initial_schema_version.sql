...
CREATE TABLE text_attribute (
  ...
  index integer,
  CONSTRAINT text_attribute_pkey PRIMARY KEY (graph_id, domain_id, id),
  CONSTRAINT text_attribute_id_check CHECK (id ~ '^[A-Za-z0-9_\\-]+$'),
  CONSTRAINT text_attribute_domain_fkey
    FOREIGN KEY (graph_id, domain_id)
    REFERENCES type(graph_id, id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT text_attribute_uri_unique UNIQUE (graph_id, domain_id, uri),
  CONSTRAINT text_attribute_regex_unique UNIQUE (graph_id, domain_id, id, regex)
);

CREATE INDEX text_attribute_graph_id_idx
    ON text_attribute(graph_id);
...
CREATE TABLE reference_attribute (
  ...
  index integer,
  CONSTRAINT reference_attribute_pkey PRIMARY KEY (graph_id, domain_id, id),
  CONSTRAINT reference_attribute_domain_fkey
    FOREIGN KEY (graph_id, domain_id)
    REFERENCES type(graph_id, id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT reference_attribute_uri_unique UNIQUE (graph_id, domain_id, uri),
  CONSTRAINT reference_attribute_range_fkey
    FOREIGN KEY (range_graph_id, range_id)
    REFERENCES type(graph_id, id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT reference_attribute_range_unique UNIQUE (graph_id, domain_id, id, range_graph_id, range_id),
  CONSTRAINT reference_attribute_id_check CHECK (id ~ '^[A-Za-z0-9_\\-]+$')
);

CREATE INDEX reference_attribute_graph_id_idx
    ON reference_attribute(graph_id);
...
CREATE TABLE node_text_attribute_value (
  ...
  CONSTRAINT node_text_attribute_value_pkey
    PRIMARY KEY (graph_id, node_type_id, node_id, attribute_id, index),
  CONSTRAINT node_text_attribute_value_node_fkey
    FOREIGN KEY (graph_id, node_type_id, node_id)
    REFERENCES node(graph_id, type_id, id) ON DELETE CASCADE,
  CONSTRAINT node_text_attribute_value_attribute_fkey
    FOREIGN KEY (graph_id, node_type_id, regex, attribute_id)
    REFERENCES text_attribute(graph_id, domain_id, regex, id) ON UPDATE CASCADE,
...
CREATE INDEX node_text_attribute_value_node_idx
    ON node_text_attribute_value(graph_id, node_type_id, node_id);
...
CREATE TABLE node_reference_attribute_value (
  ...
  CONSTRAINT node_reference_attribute_value_attribute_fkey
    FOREIGN KEY (graph_id, node_type_id, value_graph_id, value_type_id, attribute_id)
    REFERENCES reference_attribute(graph_id, domain_id, range_graph_id, range_id, id) ON UPDATE CASCADE,
...
CREATE INDEX node_reference_attribute_value_node_idx
    ON node_reference_attribute_value(graph_id, node_type_id, node_id);
